...
float impulsCointDay = 0;           // Количество импульсов
...
float moneyMonth = 0;               // стоимость за месяц
int korrect = 0;
int btnKorrect = 0;
...
Blynk.connect();  // а потом пробуем уже конектиться
  ...
  EEPROM.get(3, dayWat);      // Считать с ячейки 2 максимальное значение ВаттdayWat
}
...
void impuls() {
  ...
  if (statePin != statePinOld && millis() - lastImpuls > 50) {    // Если состояние пина изменилось (с антидребезгом)
    if (statePin == HIGH) {                                       // И если индикатор горит
      ...
      impulsCointDay = impulsCointDay + 1;
...
void timerData() {
  if (millis() - lastSaveData > 300000) {   // Если прошло 5 минут
    ...
    EEPROM.put(3, dayWat);                  // Сохранить в ячейку 3 максимальное значение Ватт
    wat = (impulsCoint * 1000) / 3200;                          // Сколько Ватт использованно
    dayWat = ((impulsCointDay * 1000) / 3200) / 1000;           // Количество Киловатт за сутки
    ...
    Blynk.virtualWrite(V6, moneyMonth);                         // Отправить стоимость за месяц
...
void timeReset() {                         // Обнулить показания за сутки и месяц
  if (hour() == 0 && minute() == 0) {
    moneyMonth = moneyMonth + (dayWat * 4.33);
    maxWattage = 0;
    impulsCointDay = 0;
    moneyDay = 0;
    dayWat = 0;
